...
#define task 2 // 1 for robot arm touching the object, 2 for gripper base touching the object 
...
#define NUM_ACTIONS  6 //two action for each joint 
/*
/ TODO - Define Reward Parameters
/
*/

#define REWARD_WIN         20.0f //300.0f
#define REWARD_LOSS       -20.0f
#define REWARD_MULTIPLIER  4.0f  //muliplier for interim reward/
#define alpha              0.5f //used for moving average of the delta of the distance to the goal /
// Define Object Names
...
#define COLLISION_FILTER "ground_plane::link::collision"   //used for detecting collison with ground 
#define COLLISION_ITEM   "tube::tube_link::tube_collision" // used for detecting collision with the prop 
#define COLLISION_POINT  "arm::gripperbase::gripper_link"  // used for detecting collision with gripper's bas
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	{       //detect collison between  link 2 and ground 
		...
		// TODO - Check if there is collision between the arm and object, then issue learning reward
                if (task ==1) 
                 {
                   if( strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0 ) // if the arm is making collision with the object 
		...
		{      
                 if( strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0 )
                    {
                      if( strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0 )//gripper is making collison with the object 
                         {       //reward for collision 
                            collision = true; 
                         }
                     else // arm is making collison but the gripper doesn't make collsion with the object,penalty should be issued w.r.t task 2
                         {
                  
		            rewardHistory = REWARD_LOSS;
                            newReward = true;
                            endEpisode = true ;	// end episode because this is not the goal of task 2 
                         }
		    }
...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	// TODO - Increase or decrease the joint position based on whether the action is even or odd
        const int j = action/2;
        float joint = ref[j] +actionJointDelta * ((action % 2 == 0) ? 1.0f :-1.0f);
       /*
        if (action%2 ==0)
          { float joint = ref[j] + actionJointDelta ; } // TODO - Set joint position based on whether action is odd.
        else
	  { float joint = ref[j] - actionJointDelta;} // TODO - Set joint position based on whether action is odd.
        */
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		// TODO - Issue an interim reward based on the distance to the objec		
		else
		{
			...
			if( episodeFrames > 1 )
			{
				...
				rewardHistory = (avgGoalDelta)*REWARD_MULTIPLIER -0.2 ;
				newReward     = true;	
			}
